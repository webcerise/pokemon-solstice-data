(function () {
  ...
  function openPokemon(pokemon) {
    ...
    const evolution = el("section", "solstice-pokedex-detail-section"); evolution.appendChild(el("h3", "", "Évolution et reproduction")); const trainerLevel = pokemon.evolution?.trainerLevel; const evolutionRequirement = pokemon.evolution?.requirement; const requirementText = evolutionRequirement && pokemon.evolution?.method === "item" ? ` ${evolutionRequirement} requise.` : evolutionRequirement && pokemon.evolution?.method === "trade" ? ` ${evolutionRequirement} requis.` : ""; const evolutionText = (trainerLevel ? `Niveau de dresseur requis : ${trainerLevel}.` : "Aucun niveau de dresseur particulier n’est requis.") + requirementText; evolution.appendChild(el("p", "", evolutionText)); evolution.appendChild(el("p", "", pokemon.gender.genderless ? "Asexué" : pokemon.gender.femaleRate === null ? "Répartition inconnue" : `${pokemon.gender.femaleRate} % de femelles`)); ui.detail.appendChild(evolution);
